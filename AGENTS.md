...
- Key directories:
  ...
  - `lib/db.ts` — PrismaClient singleton (LibSQL adapter); `prisma/schema.prisma` — ChatSession (optional user `title`) + ChatMessage + UserPreference (profile, presets, and the Google Calendar service-account key/calendar id stored for `schedule_block`); `app/actions.ts` — Server Actions for per-session persistence (FR-11) + `listChatSessions` (search + paginated sidebar list) / `renameChatSession`, plus `updateUserPreferences` (validates a pasted service-account key JSON before saving) and `testGoogleCalendarConnection` (token exchange + calendar access check)
