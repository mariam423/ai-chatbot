...
export default function ChatApp() {
  ...
  // Guards against a double-click on "Show more" double-appending a page.
  ...
  // Debounced search: typing in the sidebar box reloads page 0 after a pause.
  ...
  /* eslint-disable react-hooks/set-state-in-effect, react-hooks/exhaustive-deps --
     Intentional: reading the persisted session id and theme into state after
     mount is the SSR-safe alternative to lazy initializers (no window access
     during render); `ready` gates the chat on that restore completing; the
     session list is loaded once here (refreshSessions is stable). */
  useEffect(() => {
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-white dark:bg-zinc-950">
      ...
      <main id="main" className="flex min-w-0 flex-1 flex-col">
        <header className="flex items-center justify-between border-b border-zinc-100 bg-white/80 px-4 py-3 backdrop-blur-md dark:border-zinc-800/60 dark:bg-zinc-950/80">
          <div className="flex items-center gap-2">
            <motion.button
              ...
              className="flex size-9 items-center justify-center rounded-xl text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-900 md:hidden dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-100"
            >
              <HugeiconsIcon icon={MenuIcon} size={20} strokeWidth={1.5} />
            </motion.button>
            <div className="flex items-center gap-2">
              <div className="flex size-7 items-center justify-center rounded-lg bg-gradient-to-br from-violet-500 to-indigo-600 shadow-[0_2px_8px_rgba(124,58,237,0.3)]">
          ...
          <div className="flex items-center gap-1">
            ...
            {session?.user && (
              <div className="hidden items-center gap-2 md:flex">
                <div className="flex items-center gap-2 rounded-xl border border-zinc-100 bg-zinc-50/80 px-2 py-1 dark:border-zinc-800/60 dark:bg-zinc-900/60">
                  {session.user.image ? (
                    <Image
            ...
            <div className="flex items-center gap-1 md:hidden">
              <motion.button
                ...
                className="flex size-9 items-center justify-center rounded-xl text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-100"
              >
