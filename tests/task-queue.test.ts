...
beforeEach(async () => {
  ...
  vi.doMock('bullmq', () => ({
    Queue: vi.fn().mockImplementation(function () { return fakeQueue }),
  }))
...
describe('addTask', () => {
  ...
  it('returns null when Redis is unavailable', async () => {
    ...
    vi.doMock('../lib/redis', () => ({
      requireRedis: vi.fn(() => { throw new Error('REDIS_URL not set') }),
  ...
  it('handles queue creation failure gracefully', async () => {
    vi.doMock('bullmq', () => ({
      Queue: vi.fn().mockImplementation(() => { throw new Error('Connection refused') }),
    }))
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {})

    const { addTask } = await import('../lib/queues/task-queue')
    const jobId = await addTask('document:process', {
      sessionId: 's1', documentId: 'd1', userId: 'u1', fileName: 'test.pdf',
    })
    expect(jobId).toBeNull()
