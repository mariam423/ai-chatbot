...
check(
  ...
  sessionCookie ? `${sessionCookie.value.length} chars` : 'MISSING',
)
...
if (composer) {
  ...
  if (send) {
    ...
    await send.click()
    try {
      await page.waitForFunction(() => (document.body.textContent ?? '').includes('PONG'), {
        timeout: 30_000,
      })
...
check(
  'stop/regenerate control present',
  Boolean(stopBtn) || Boolean(regen),
...
check(
  'assistant reply includes <pre><code>',
  codeBlocks.length > 0,
...
if (docInput) {
  ...
  try {
    ...
    try {
      await page.waitForFunction(() => (document.body.textContent ?? '').includes('qa-rag.txt'), {
        timeout: 15_000,
      })
    ...
    if (composer2) {
      ...
      await send2?.click()
      try {
        await page.waitForFunction(() => (document.body.textContent ?? '').includes('Coralhaven'), {
          timeout: 30_000,
        })
        check('RAG retrieved city from uploaded doc', true)
      } catch {
        check(
          'RAG retrieved city from uploaded doc',
          false,
...
check(
  'citation drawer markup present (or just-not-shown-yet)',
  true,
...
const themeBtn = await page.$(
  'button[aria-label*="dark mode" i], button[aria-label*="light mode" i]',
)
...
check('settings page rendered', settingsBody.length > 100, `${settingsBody.length} chars`)

step('15. Dashboard renders')
await page.goto(`${APP_URL}/dashboard`, { waitUntil: 'networkidle' })
const dashBody = (await page.textContent('body')) ?? ''
check('dashboard page rendered', dashBody.length > 100, `${dashBody.length} chars`)
...
const expectedBad =
  bad - (apiCalls.find((c) => c.path === '/api/transcribe' && c.status === 429) ? 1 : 0)
if (expectedBad > 0) {
